...
describe("AppController", () => {
  ...
  describe("getTaxPosition", () => {
    ...
    it("Should return tax position of -100 when 1 tax payment is returned", async () => {
      const date = "2026-08-03T12:30:00Z"
      mockDataBaseService.readByDate.mockResolvedValue({ success: true, records: { [appService.EventTypes.sales]: [], [appService.EventTypes.taxPayment]: [{ date: date, amount: 100 }] } })
      const res = await appController.getTaxPosition({ date: date })
      ...
      mockDataBaseService.readByDate.mockResolvedValue({
        success: true, records: {
          [appService.EventTypes.sales]: [{
            date: date,
            invoiceId: "123456",
            items: [{ itemId: "Item1", cost: 100, taxRate: 0.5 }, { itemId: "Item2", cost: 500, taxRate: 0.2 }]
          }],
      ...
      mockDataBaseService.readByDate.mockResolvedValue({
        success: true, records: {
          [appService.EventTypes.sales]: [{
            date: date,
            invoiceId: "1",
            items: [{ itemId: "Item1", cost: 100, taxRate: 0.1 }, { itemId: "Item2", cost: 500, taxRate: 0.2 }]
          },
          {
            date: date,
            invoiceId: "2",
            items: [{ itemId: "Item1", cost: 1000, taxRate: 0.5 }, { itemId: "Item2", cost: 900, taxRate: 0.2 }],
            amendments: [
              { date: date, itemId: "Item1", cost: 100, taxRate: 0.5 },
              { date: date, itemId: "Item2", cost: 100, taxRate: 0.5 }
            ]
          }
          ], [appService.EventTypes.taxPayment]: [{ date: date, amount: 100 }, { date: date, amount: 100 }]
        }
      })
      const res = await appController.getTaxPosition({ date: date })
          ...
          [appService.EventTypes.sales]: [
            {
              ...
              amendments: [
                { date: "2026-08-04T10:30:00Z", itemId: "Item1", cost: 2000, taxRate: 0.5 }, // This amendment should be used
                { date: "2026-08-03T13:30:00Z", itemId: "Item1", cost: 100, taxRate: 0.5 }
              ]
            }
          ], [appService.EventTypes.taxPayment]: [{ date: "2026-08-03T12:30:00Z", amount: 1000 }]
        }
      })
      const res = await appController.getTaxPosition({ date: "2026-08-03T12:30:00Z" })
      expect(res.taxPosition).toBe(0)
    });
          ...
          [appService.EventTypes.sales]: [
            {
              ...
              amendments: [
                { date: "2026-08-04T10:30:00Z", itemId: "Item2", cost: 2000, taxRate: 0.5 }, // This amendment should be used
              ]
            }
  ...
  describe("createTransation", () => {
    describe("Tax Payments", () => {
      it("Should create a taxPayment for a given date", async () => {
        ...
        expect(mockDataBaseService.create).toHaveBeenCalledWith("TAX_PAYMENT", "2026-08-03T12:30:00Z", { "amount": 1500, "date": "2026-08-03T12:30:00Z", "eventType": "TAX_PAYMENT" })
      });
      it("Should raise a ConflictException if a taxpayment for that date already exists ", async () => {
        mockDataBaseService.create.mockResolvedValue({ success: false, error: { message: "Conflict", code: 409 } })
        await expect(
    ...
    describe("Sale Events", () => {
      ...
      it("Should update sale event created by an amendment for an item the sale event doent have", async () => {
        mockDataBaseService.readById.mockResolvedValue({
          success: true, record: {
            eventType: "SALES",
            invoiceId: "Invoice 1",
            amendments: [{ date: "2026-08-03T12:31:00Z", "itemId": "Item 3", "cost": 40000, "taxRate": 0.15 }]
          }
        })
      ...
      it("Should raise Conflict error when a sale event created a sale event is found", async () => {
        ...
        await expect(appController.createTransation({
          eventType: "SALES",
          date: "2026-08-03T12:30:00Z",
          invoiceId: "Invoice 1",
          items: [{ itemId: "Item 1", cost: 1099, taxRate: 0.2 }]
  ...
  describe("amendSale", () => {
    it("Should update existing sale event with amendment", async () => {
      ...
      expect(mockDataBaseService.update).toHaveBeenCalledWith("SALES", "Invoice 1",
        {
          amendments: [{ cost: 40000, date: "2026-08-03T17:29:39Z", itemId: "Item 1", taxRate: 0.15 }],
          date: "2026-08-03T12:30:00Z",
          eventType: "SALES",
          invoiceId: "Invoice 1",
          items: [{ cost: 1099, itemId: "Item 1", taxRate: 0.2 }]
